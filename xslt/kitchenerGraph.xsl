...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="$gal_text">
        <xsl:result-document method="xhtml" indent="yes" href="../docs/kitchenerGraph.html">
            <html>
                <head><title>Mentions of Kitchener by Chapter</title></head>
                <body>
                    ...
                    <svg viewBox="0 0 2400 1000">
                        <g transform="translate(425,50)">
                            <xsl:for-each select="//chapter">
                                ...
                                <xsl:variable name="kitchener-count" select=".//person[@name='Herbert Kitchener']=>count()"/>
                                ...
                                <text font-weight="bold" font-size="25" x="{$xspacer * $kitchener-count + 20}" y="{$yspacer * $chapNum + 6}" text-anchor="right">
                                    <xsl:value-of select="$kitchener-count"/></text>
                    ...
                    <p>This graph shows the frequency with which Kitchener is mentioned, by chapter.</p>
                </body>
            </html>
        </xsl:result-document>
    </xsl:template>
